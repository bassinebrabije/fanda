...
function Buy() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const filteredArticles = allArticles.filter(article => {
                ...
                const matchesVille = article.typevend === 'Vendre'; // Check if ville is 'casablanca'
    ...
    return (
        <>
            <Helmet>
                <title>Acheter</title>
                <link rel="canonical" href="https://prestigefandaimmobilier.com/Acheter" />
            </Helmet>
                    ...
                    <div className="mt-10 grid max-w-md grid-cols-1 gap-6 px-2 sm:max-w-lg sm:px-20 md:max-w-screen-xl md:grid-cols-2 md:px-10 lg:grid-cols-3 lg:gap-8">
                        {articles.length > 0 ? (
                            ...
                            <div className="bg-red-200 px-6  py-4 mx-2 my-4 rounded-md text-lg flex items-center mx-auto max-w-lg">
                                <span className="text-red-800">  Aucun bien ne correspond au type sélectionné.</span>
                            </div>
